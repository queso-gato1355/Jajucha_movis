...
class Planning(BasePlanning):
    def __init__(self, graphics):
        ...
        self.vars.steer_modify = 6 # 자주차가 직진하는 값으로, 서보모터의 상태에 따라 자주차의 steer 값이 0 임에도 직진하지 않는 경우가 있다.
                                   # 따라서, 이를 조정하여 자주차가 직진하도록 만들어야 한다. 양수는 오른쪽, 음수는 왼쪽이다.
    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        for i in range(len(frontLines)):
            if frontLines[i][0, 1] < 320: # (1) [No 460]은 적절한 값이 아닙니다!! / y1, y2, y3... [No 460]보다 작은가? 그러니까, 차선이 나타는 부분까지만 이미지를 자르자고 하는거야. 어디까지 잘라야 하는걸까?
            ...
            if line(400) < center_x:   # (2) [No 450]은 적절한 값이 아닙니다!! / 
        ...
        if line[0] == 'right':              # line 이 우차선이면
            ...
            e = line(420) - center_x - 250  # (2) [No 450]과 (3) [No 100]은 적절한 값이 아닙니다!!
            ...
            print('line(420)', line(420)) # (2) [No 450]의 적절한 값을 넣어주세요.
            ...
            coefficient = np.polyfit(y, x, 1)
            line = np.poly1d(coefficient)
            e = line(420) - center_x + 250  # (2) [No 450]과 (4) [No 110]은 적절한 값이 아닙니다!!
            # 추세선 line = a*[No 450] + b 에서 line([No 450])은 추세선에서 y=[No 450]일 때의 x의 값을 의미하며
        ...
        steer = 1* e + self.vars.steer_modify # (5) [No 2]와 (6) [No 3]은 적절한 값이 아닙니다. steer는 조향값이며, e는 픽셀의 차이입니다.
