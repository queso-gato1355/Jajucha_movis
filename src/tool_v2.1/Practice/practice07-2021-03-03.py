...
class Planning(BasePlanning):
    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        if line is None:                    # line 이 없으면
            ...
            imgBackWarpPoints = utlis.drawPoints(rearImage.copy(),points)#뒤쪽 카메라 처리도 새로운 point 규격을 필요로 하고 있음. 새로 찾기를 희망함.
            #cv2.imshow('Back Warp Points', imgFrontWarpPoints)

            imgWarpOriginalRear = utlis.warpImg(rearImage.copy(), points, self.vars.wT, self.vars.hT)
