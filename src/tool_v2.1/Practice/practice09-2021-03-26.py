...
class Planning(BasePlanning):
    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        points = np.float32([(50, 300), (self.vars.wT-50, 300),(-600 , 640), (self.vars.wT+750, 640)])
        rearPoints = np.float32([(self.vars.wT+24, 300),(self.vars.wT-320, 300), (self.vars.wT+765, 640), (self.vars.wT-810, 640)])
        ...
        image = Image.fromarray(canny).convert('RGB')
        ...
        if line is None:                    # line 이 없으면
            ...
            if rearline[0] == 'right':              # line 이 우차선이면
                self.vars.Is_It_time_to_choose_back_camera = True
                ...
                theImageRear = utlis.drawLines(laneImageBack, self.vars.rearImage_line_detect_x_value_right, center_x, e_rear, rearline)
                ...
                print('이미지 가운데 좌표 :', center_x, '/ x축 값과 가운데 좌표간 픽셀 차이 보정 :', e_rear)
                theImageRear = utlis.drawLines(laneImageBack, self.vars.rearImage_line_detect_x_value_left, center_x, e_rear, rearline)
            ...
            if e_rear >= -self.vars.steer_modify and e_rear <= self.vars.steer_modify:
                ...
                steer = utlis.steerFunction(self.vars.rear_tilt, e_rear, self.vars.steer_modify, self.vars.chatsu, True)
            elif e_rear > self.vars.steer_modify:
                steer = utlis.steerFunction(self.vars.rear_tilt, e_rear, self.vars.steer_modify, self.vars.chatsu, False)
            ...
            imgWarpOriginalRear = utlis.warpImg(cannyRear.copy(), rearPoints, self.vars.wT, self.vars.hT)
            cv2.imshow('Back Warped Image', imgWarpOriginalRear)
            cv2.putText(theImageRear, "steer = " + str(int(steer)), (478,440), font, 0.7,(0,255,0),1)
            #cv2.imshow('Back Lane Image', theImageRear)
        ...
        if line[0] == 'right':              # line 이 우차선이면
            ...
            theImage = utlis.drawLines(laneImage, self.vars.frontImage_line_detect_x_value_right, center_x, e, line)
        else:
            self.vars.Is_It_time_to_choose_back_camera = False
            ...
            print('이미지 가운데 좌표 :', center_x, '/ x축 값과 가운데 좌표간 픽셀 차이 보정 :', e)
            theImage = utlis.drawLines(laneImage, self.vars.frontImage_line_detect_x_value_left, center_x, e, line)


        if e >= -self.vars.steer_modify and e <= self.vars.steer_modify:
            steer = self.vars.steer_modify
        elif e < -self.vars.steer_modify:
            steer = utlis.steerFunction(self.vars.front_tilt, e, self.vars.steer_modify, self.vars.chatsu, True)
        elif e > self.vars.steer_modify:
            steer = utlis.steerFunction(self.vars.front_tilt, e, self.vars.steer_modify, self.vars.chatsu, False)
        ...
        imgWarpOriginal = utlis.warpImg(canny.copy(), points, self.vars.wT, self.vars.hT)
        cv2.imshow('Warped Image', imgWarpOriginal)

        #imgWarpOriginalRear = utlis.warpImg(cannyRear.copy(), rearPoints, self.vars.wT, self.vars.hT)
        #cv2.imshow('Back Warped Image', imgWarpOriginalRear)

        #imgBackWarpPoints = utlis.drawPoints(cannyRear.copy(),rearPoints, font)#뒤쪽 카메라 처리도 새로운 point 규격을 필요로 하고 있음. 새로 찾기를 희망함.
        #cv2.imshow('Back Warp Points', imgBackWarpPoints)
        ...
        for i in range(0, 2):
        	ap.append(int(prediction[0][i] * 100))

        print("prediction : ", prediction)

        print("좌회전 :", int(prediction[0][0] * 100))
        print("직진 :", int(prediction[0][1] * 100))
        print("우회전 :", int(prediction[0][2]* 100))
