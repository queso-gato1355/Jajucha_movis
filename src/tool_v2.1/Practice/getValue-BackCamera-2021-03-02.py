...
(1) "차선 선택"에서
    예) if rearLines[i][0, 1] < 460: # [No 460]은 적절한 값이 아닙니다!!
...
class Planning(BasePlanning):
    def __init__(self, graphics):
        ...
        self.vars.image_line_detect_x_value_right = 300 # 이미지가 인식할 추세선 위 파란 점의 위치를 정하는 값. 값이 작을 수록 이미지 상단의 파란 점으로 설정된다.
        self.vars.image_line_detect_x_value_final = 450 # 왼쪽으로 기준선을 정할지, 오른쪽으로 정할지 결정하기 위한 기준점으로,
                                                        # 가장 아래쪽에 위치한 직선을 기준으로 왼쪽선을, 혹은 오른쪽선을 기준으로 잡을지가 결정된다.
        self.vars.image_line_detect_x_value_left = 300  # 왼쪽으로 기준선이 결정되었을 때 왼쪽 추세선 위의 어느 점을 고를지 결정. 300이상을 추천함
        self.vars.line_and_middle_line_pixel_difference_right = 100 # 각 추세선 위의 파란점과 중앙으로부터의 픽셀 차이. 이 값을 조정하여 차량의 위치를 조정한다.
                                                                    # 값이 작을수록 차량이 인식하는 차선(오른쪽, 왼쪽)에 가까이 붙으며, 값이 클수록 인식하는 차선으로부터 멀어진다. 
        self.vars.line_and_middle_line_pixel_difference_left = 12  # 위와 동일

        self.vars.tilt = 0.00001                                      # steer 값을 조정하는 함수의 최고차항의 계수
    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        # rearLines = [[x1,y1],[x2,y2],[x3,y3], … ] , [[x1,y1], [x2,y2], … ]
        # rearLines[0]이 가장 왼쪽 차선, 빨(0), 주(1), 노(2), 초(3), 파(4), 남(5), 보(6)
        ...
        # canny image 출력 
        canny = self.canny(rearImage)
        self.imshow('Canny Image(rear)', canny)
        ...
        # rearLines를 정렬(x[0,1]을 기준으로 내림차순으로 정렬)
        # rearLines = [[x1,y1],[x2,y2],[x3,y3], … ] , [[x1,y1], [x2,y2], … ]
        # y1>y2>y3... : 밑의 점부터 표현
        # print ('rearLines=', rearLines)
        for i in range(len(rearLines)) :
            if rearLines[i][0, 1] < 340 : # (1) [No 460]은 적절한 값이 아닙니다!! / y1, y2, y3... [No 460]보다 작은가? 그러니까, 차선이 나타는 부분까지만 이미지를 자르자고 하는거야. 어디까지 잘라야 하는걸까?
                continue              # y값이 [No 460]보다 작으면 추세선 안하고 넘어가라(479~[No 460]까지만 추세선 찾기)
            x = rearLines[i][:, 0]    # x=[x1, x2, x3, ...]
            y = rearLines[i][:, 1]    # y=[y1, y2, y3, ...]
            ...
            if line(self.vars.image_line_detect_x_value_final) < center_x:   # (2) [No 450]은 적절한 값이 아닙니다!! / 
                ...
                break
        # print('line', line)
        laneImage = rearImage.copy()
        if line is None:                    # line 이 없으면
            # No line
            print('차선이 감지되지 않습니다! 이전 명령어로 작동합니다.')



            #steer = self.vars.tilt * e + self.vars.steer_modify # (5) [No 2]와 (6) [No 3]은 적절한 값이 아닙니다. steer는 조향값이며, e는 픽셀의 차이입니다.
            if self.vars.steer < -60:
                print("조향 : 급격한 좌회전")
                
            elif self.vars.steer >= -60 and self.vars.steer < -40:
                print("조향 : 거센 좌회전")
                
            elif self.vars.steer >= -40 and self.vars.steer < -20:
                print("조향 : 조금 거센 좌회전")
                
            elif self.vars.steer >= -20 and self.vars.steer <-10 :
                print("조향 : 약한 좌회전")
            ...
            if self.vars.steer > - 10 and self.vars.steer < 10 :
        ...
        if line[0] == 'right':              # line 이 우차선이면
            ...
            line = rearLines[line[1]]    # 다시 line 재정의 
            x = line[:, 0]
            y = line[:, 1]
            ...
            cv2.circle(laneImage, (int(line(self.vars.image_line_detect_x_value_right)),self.vars.image_line_detect_x_value_right), 7, (255, 0, 0), -1)
            cv2.circle(laneImage, (int(line(self.vars.image_line_detect_x_value_right)),self.vars.image_line_detect_x_value_right), 5, (0, 255, 0), -1)

            cv2.line(laneImage, (int(line(self.vars.image_line_detect_x_value_right)), self.vars.image_line_detect_x_value_right - 20), (int(line(self.vars.image_line_detect_x_value_right)), self.vars.image_line_detect_x_value_right + 20),(0, 255, 0), 2)

            cv2.line(laneImage, (center_x, self.vars.image_line_detect_x_value_right), (int(line(self.vars.image_line_detect_x_value_right)),self.vars.image_line_detect_x_value_right), (0, 0, 0), 4)
            cv2.line(laneImage, (center_x, self.vars.image_line_detect_x_value_right), (int(line(self.vars.image_line_detect_x_value_right)),self.vars.image_line_detect_x_value_right), (255, 0, 0), 2)

            font=cv2.FONT_HERSHEY_SIMPLEX

            cv2.putText(laneImage, str(int(line(self.vars.image_line_detect_x_value_right) - center_x)), (int((center_x + line(self.vars.image_line_detect_x_value_right))/2) - 30, int(self.vars.image_line_detect_x_value_right) - 10), font, 1,(0,0,0),2)
            cv2.putText(laneImage, "e = " + str(int(e)), (520,460), font, 0.7,(0,255,0),1)

            #cv2.imshow('Back Lane Image', laneImage)
        else:
            print('====왼쪽 차선 기준====')
            # follow left
            line = rearLines[line[1]]
            x = line[:, 0]
            y = line[:, 1]
            coefficient = np.polyfit(y, x, 1)
            line = np.poly1d(coefficient)
            ...
            cv2.circle(laneImage, (int(line(self.vars.image_line_detect_x_value_left)),self.vars.image_line_detect_x_value_left), 7, (255, 0, 0), -1)
            cv2.circle(laneImage, (int(line(self.vars.image_line_detect_x_value_left)),self.vars.image_line_detect_x_value_left), 5, (0, 255, 0), -1)

            cv2.line(laneImage, (int(line(self.vars.image_line_detect_x_value_left)), self.vars.image_line_detect_x_value_left - 20), (int(line(self.vars.image_line_detect_x_value_left)), self.vars.image_line_detect_x_value_left + 20),(0, 255, 0), 2)

            cv2.line(laneImage, (center_x, self.vars.image_line_detect_x_value_left), (int(line(self.vars.image_line_detect_x_value_left)),self.vars.image_line_detect_x_value_left), (0, 0, 0), 4)
            cv2.line(laneImage, (center_x, self.vars.image_line_detect_x_value_left), (int(line(self.vars.image_line_detect_x_value_left)),self.vars.image_line_detect_x_value_left), (255, 0, 0), 2)

            font=cv2.FONT_HERSHEY_SIMPLEX

            cv2.putText(laneImage, str(abs(int(line(self.vars.image_line_detect_x_value_left) - center_x))), (int((center_x + line(self.vars.image_line_detect_x_value_left))/2) - 30, int(self.vars.image_line_detect_x_value_left) - 10), font, 1,(0,0,0),2)
            cv2.putText(laneImage, "e = " + str(int(e)), (520,460), font, 0.7,(0,255,0),1)
            #cv2.imshow('Back Lane Image', laneImage)
        ...
        if e >= -self.vars.steer_modify and e <= self.vars.steer_modify:
            ...
            steer = self.vars.tilt * ((e + self.vars.steer_modify) ** self.vars.chatsu) + self.vars.steer_modify
        elif e > self.vars.steer_modify:
            steer = self.vars.tilt * ((e - self.vars.steer_modify) ** self.vars.chatsu) + self.vars.steer_modify

        #steer = self.vars.tilt * e + self.vars.steer_modify # (5) [No 2]와 (6) [No 3]은 적절한 값이 아닙니다. steer는 조향값이며, e는 픽셀의 차이입니다.self.vars.steer_modify
        self.vars.steer = steer
        print('steer : ', steer)

        cv2.putText(laneImage, "steer = " + str(int(steer)), (478,440), font, 0.7,(0,255,0),1)

        cv2.imshow('Lane Image', laneImage)

        if steer < -60 + self.vars.steer_modify:
            print("조향 : 급격한 좌회전")
        elif steer >= -60 + self.vars.steer_modify and steer < -40 + self.vars.steer_modify:
            print("조향 : 거센 좌회전")
        elif steer >= -40 + self.vars.steer_modify and steer < -20 + self.vars.steer_modify:
            print("조향 : 조금 거센 좌회전")
        elif steer >= -20 + self.vars.steer_modify and steer < -10 + self.vars.steer_modify:
            print("조향 : 약한 좌회전")
        elif steer <= 20 + self.vars.steer_modify and steer > 10 + self.vars.steer_modify:
            print("조향 : 약한 우회전")
        elif steer <= 40 + self.vars.steer_modify and steer > 20 + self.vars.steer_modify :
            ...
            print("조향 : 거센 우회전")
        elif steer > 60 + self.vars.steer_modify:
            print("조향 : 급격한 우회전")
        else:
            print("조향 : 직진")

        if self.vars.steer > 80:
            self.vars.steer = 80
        elif self.vars.steer < -80:
            self.vars.steer = -80
