...
class Planning(BasePlanning):
    def __init__(self, graphics):
        ...
        self.vars.frontImage_line_detect_x_value_right = 390 # 이미지가 인식할 추세선 위 파란 점의 위치를 정하는 값. 값이 작을 수록 이미지 상단의 파란 점으로 설정된다.
        ...
        self.vars.frontImage_line_detect_x_value_left = 340  # 왼쪽으로 기준선이 결정되었을 때 왼쪽 추세선 위의 어느 점을 고를지 결정. 300이상을 추천함
        self.vars.front_line_and_middle_line_pixel_difference_right = 240 # 각 추세선 위의 파란점과 중앙으로부터의 픽셀 차이. 이 값을 조정하여 차량의 위치를 조정한다.
                                                                    # 값이 작을수록 차량이 인식하는 차선(오른쪽, 왼쪽)에 가까이 붙으며, 값이 클수록 인식하는 차선으로부터 멀어진다. 
        self.vars.front_line_and_middle_line_pixel_difference_left = 260  # 위와 동일

        self.vars.front_tilt = 0.001                                      # steer 값을 조정하는 함수의 최고차항의 계수

        self.vars.rearImage_line_detect_x_value_right = 390 # 이미지가 인식할 추세선 위 파란 점의 위치를 정하는 값. 값이 작을 수록 이미지 상단의 파란 점으로 설정된다.
        self.vars.rearImage_line_detect_x_value_final = 400 # 왼쪽으로 기준선을 정할지, 오른쪽으로 정할지 결정하기 위한 기준점으로,
                                                        # 가장 아래쪽에 위치한 직선을 기준으로 왼쪽선을, 혹은 오른쪽선을 기준으로 잡을지가 결정된다.
        self.vars.rearImage_line_detect_x_value_left = 340  # 왼쪽으로 기준선이 결정되었을 때 왼쪽 추세선 위의 어느 점을 고를지 결정. 300이상을 추천함
        self.vars.rear_line_and_middle_line_pixel_difference_right = 240 # 각 추세선 위의 파란점과 중앙으로부터의 픽셀 차이. 이 값을 조정하여 차량의 위치를 조정한다.
                                                                    # 값이 작을수록 차량이 인식하는 차선(오른쪽, 왼쪽)에 가까이 붙으며, 값이 클수록 인식하는 차선으로부터 멀어진다. 
        self.vars.rear_line_and_middle_line_pixel_difference_left = 260  # 위와 동일

        self.vars.rear_tilt = 0.0001                                      # steer 값을 조정하는 함수의 최고차항의 계수
    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        if line is None:                    # line 이 없으면
            ...
            if rearline[0] == 'right':              # line 이 우차선이면
                ...
                e_rear = rearline(self.vars.rearImage_line_detect_x_value_right) - center_x + self.vars.rear_line_and_middle_line_pixel_difference_right  # (2) [No 450]과 (3) [No 100]은 적절한 값이 아닙니다!!
                ...
                print('직선의 기울기 :', rearCoefficient[0])
                e_rear = rearline(self.vars.rearImage_line_detect_x_value_left) - center_x - self.vars.rear_line_and_middle_line_pixel_difference_left  # (2) [No 450]과 (4) [No 110]은 적절한 값이 아닙니다!!
                # 추세선 line = a*[No 450] + b 에서 line([No 450])은 추세선에서 y=[No 450]일 때의 x의 값을 의미하며
            ...
            if e_rear >= -self.vars.steer_modify and e_rear <= self.vars.steer_modify:
                ...
                steer = self.vars.rear_tilt * ((e_rear + self.vars.steer_modify) ** self.vars.chatsu) + self.vars.steer_modify
            elif e_rear > self.vars.steer_modify:
                steer = self.vars.rear_tilt * ((e_rear - self.vars.steer_modify) ** self.vars.chatsu) + self.vars.steer_modify
        ...
        if e >= -self.vars.steer_modify and e <= self.vars.steer_modify:
            ...
            steer = self.vars.front_tilt * ((e + self.vars.steer_modify) ** self.vars.chatsu) + self.vars.steer_modify
        elif e > self.vars.steer_modify:
            steer = self.vars.front_tilt * ((e - self.vars.steer_modify) ** self.vars.chatsu) + self.vars.steer_modify
