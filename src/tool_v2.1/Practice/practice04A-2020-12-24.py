...
class Planning(BasePlanning):
    def __init__(self, graphics):
        ...
        self.vars.steer_modify = 9 # 자주차가 직진하는 값으로, 서보모터의 상태에 따라 자주차의 steer 값이 0 임에도 직진하지 않는 경우가 있다.
        ...
        self.vars.image_line_detect_x_value_left = 380  # 왼쪽으로 기준선이 결정되었을 때 왼쪽 추세선 위의 어느 점을 고를지 결정. 300이상을 추천함
    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        if line is None:                    # line 이 없으면
            # No line
            print('차선이 감지되지 않습니다! 이전 명령으로 작동합니다.')
