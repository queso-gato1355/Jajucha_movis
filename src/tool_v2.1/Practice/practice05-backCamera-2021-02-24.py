...
class Planning(BasePlanning):
    def __init__(self, graphics):
        ...
        self.vars.image_line_detect_x_value_left = 400  # 왼쪽으로 기준선이 결정되었을 때 왼쪽 추세선 위의 어느 점을 고를지 결정. 300이상을 추천함
        self.vars.line_and_middle_line_pixel_difference_right = 200 # 각 추세선 위의 파란점과 중앙으로부터의 픽셀 차이. 이 값을 조정하여 차량의 위치를 조정한다.
                                                                    # 값이 작을수록 차량이 인식하는 차선(오른쪽, 왼쪽)에 가까이 붙으며, 값이 클수록 인식하는 차선으로부터 멀어진다. 
        self.vars.line_and_middle_line_pixel_difference_left = 100  # 위와 동일

        self.vars.tilt = 0.0001                                      # steer 값을 조정하는 함수의 최고차항의 계수
