...
class Planning(BasePlanning):
    def __init__(self, graphics):
        ...
        self.vars.image_line_detect_x_value_right = 390 # 이미지가 인식할 추세선 위 파란 점의 위치를 정하는 값. 값이 작을 수록 이미지 상단의 파란 점으로 설정된다.
        ...
        self.vars.image_line_detect_x_value_left = 340  # 왼쪽으로 기준선이 결정되었을 때 왼쪽 추세선 위의 어느 점을 고를지 결정. 300이상을 추천함
        self.vars.line_and_middle_line_pixel_difference_right = 240 # 각 추세선 위의 파란점과 중앙으로부터의 픽셀 차이. 이 값을 조정하여 차량의 위치를 조정한다.
                                                                    # 값이 작을수록 차량이 인식하는 차선(오른쪽, 왼쪽)에 가까이 붙으며, 값이 클수록 인식하는 차선으로부터 멀어진다. 
        self.vars.line_and_middle_line_pixel_difference_left = 260  # 위와 동일
