...
class Planning(BasePlanning):
    def __init__(self, graphics):
        ...
        self.vars.image_line_detect_x_value_right = 380 # 이미지가 인식할 추세선 위 파란 점의 위치를 정하는 값. 값이 작을 수록 이미지 상단의 파란 점으로 설정된다.
        ...
        self.vars.line_and_middle_line_pixel_difference_right = 220 # 각 추세선 위의 파란점과 중앙으로부터의 픽셀 차이. 이 값을 조정하여 차량의 위치를 조정한다.
                                                                    # 값이 작을수록 차량이 인식하는 차선(오른쪽, 왼쪽)에 가까이 붙으며, 값이 클수록 인식하는 차선으로부터 멀어진다. 
        self.vars.line_and_middle_line_pixel_difference_left = 240  # 위와 동일

        self.vars.tilt = 0.0003                                      # steer 값을 조정하는 함수의 최고차항의 계수
    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        if e >= -self.vars.steer_modify and e <= self.vars.steer_modify:
            ...
            steer = self.vars.tilt * ((e + self.vars.steer_modify) ** 3) + self.vars.steer_modify
        elif e > self.vars.steer_modify:
            steer = self.vars.tilt * ((e - self.vars.steer_modify) ** 3) + self.vars.steer_modify
