    ...
    if 0 < LiDAR and LiDAR < 300 :
        ...
        if H2RD != 160 and H2LD !=160 :
            ...
            if V1D != 0 and V1D !=120 and V7D !=0 and V7D !=120:
                ...
                centerpoint_X = int((320 - H2LD + H2RD)/2) #차선의 중앙값을 구한다
                    
                print('centerpoint_X = %d' % (centerpoint_X)) #center는 카메라 기준이다(차의 위치)그리고 centerpoint는 차선의 중간
                print('%d' % (center))
        ...
        if V4D > 75 and V4D < 85  :

            command = 'S1135E'

        if V4D < 85 and V4D > 75 and V7D < 60 and V7D > 0 :
